...
urlprot="https://"
startup_config_checksum_file="/tmp/config_db.json.sha256sum"
##########################################
...
if [ "$activate" == "load" ]
  then ## Copy config to running
    ...
    sshpass -p "$defaultpwd" scp -o StrictHostKeyChecking=no /tftpboot/sonic/postscript_addons/save_config.sh $username@$host:
